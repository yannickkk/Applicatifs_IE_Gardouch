  ...
  navbarPage(windowTitle = "IE Gardouch: ", title=div(tags$a(href="https://github.com/yannickkk/Applicatifs_IE_Gardouch/blob/main/App_registre/readme.md","Documentation", target ="_blank")),
             tabPanel("Registre des entrées/sorites",  
                      fluidRow(
                        ...
                        column(width= 1,selectInput(inputId = "enclos_entree",label = labelMandatory("Enclos d'entrée"), choices=append("",utf8(dbGetQuery(con, "Select ind_name from main.t_individu_ind, list.tr_object_obj where ind_obj_id = obj_id and obj_name = 'enclos'"))[-1,1]),selected = "", selectize = TRUE, size = NULL),style = "margin-top:25px;"),
