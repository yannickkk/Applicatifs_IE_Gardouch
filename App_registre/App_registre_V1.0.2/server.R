...
###########################################################################
  ...
  formData <- reactive({
    ...
    nouvelleentree$ani_espece_id<<- "chevreuil" ###car on a désactivé l'entree de l'espece dans l'ui
    nouvelleentree
  })
...
#################################################################################################################    
        ...
        obj <- dbGetQuery(con,paste0("SELECT obj_id FROM list.tr_object_obj WHERE obj_name =  'chevreuil'")) ##",input$ani_espece_id,"
        verif<- dbGetQuery(con,paste0("Select distinct(ind_name) from main.t_individu_ind where ind_obj_id = ",obj," "))
...
#######fonction qui controle les champs et renvoie TRUE si pas d'alerte détectées et FALSE sinon (permet de gérer l'activation ou non de la sauvegarde lors du submit)  
  ...
  ##########################################################################################################################################################################################       
  ############################### si la case né à Gardouch est cochée alors de champ text de la provenance change pour devenir une liste déroulante avec les noms d'enclos #################
  ##########################################################################################################################################################################################   
