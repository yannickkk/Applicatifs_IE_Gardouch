...
# on va utiliser con
# on connecte directement la base de l'Installation experimentale de Gardouch (possible de changer user et pw)
...
#Enclos de 0 a 9, on met dans des dataframes differents
...
#Pour recuperer le sexe et le marquage de l'animal 
...
#Pour la page survey : 
#Relevant sert a faire apparaitre ou pas une question selon ce que l'on selectionne avant
...
v1 <- c('start','end','today','begin group','select_one observateur','select_one enclos','select_one nom_0','select_one nom_1','select_one nom_2','select_one nom_3','select_one nom_4','select_one nom_5','select_one nom_6','select_one nom_7','select_one nom_8','select_one nom_9','end group','begin group','note','note','note','note','note','note','note','note','note','note','note','note','date','end group','begin group','select_one posture','select_multiple comportement','select_one physique','end group','begin group','select_multiple blessure','select_one localisation','select_one feces','end group','begin group','select_multiple salissure','select_one secoue','select_one oreille','end group','begin group','select_one mouche','end group','begin group','text','text','end group','begin group','image','end group')
v2 <- c('start','end','today','single_page','observateur','enclos','nom_0','nom_1','nom_2','nom_3','nom_4','nom_5','nom_6','nom_7','nom_8','nom_9','single_page','single_page_1','gender_1','gender_2','marquage_1','marquage_2','marquage_3','marquage_4','marquage_5','marquage_6','marquage_7','marquage_8','marquage_9','marquage_10','date','single_page_1','single_page_2','posture','comportement','condition_physique','single_page_2','single_page_3','blessure','localisation','feces','single_page_3','single_page_4','salissure','secoue','oreille','single_page_4','single_page_5','mouche','single_page_5','single_page_6','description','remarque','single_page_6','single_page_7','photo','single_page_7'); 
v3 <- c(NA,NA,NA,NA,"Selectionnez l'observateur",'Enclos','Selectionnez un animal','Selectionnez un animal','Selectionnez un animal','Selectionnez un animal','Selectionnez un animal','Selectionnez un animal','Selectionnez un animal','Selectionnez un animal','Selectionnez un animal','Selectionnez un animal',NA,NA,"L'animal est un mâle","L'animal est une femelle","Le marquage de l'animal est un accelerometre noir vert v10","Le marquage de l'animal est un accelerometre noir rouge s3","Le marquage de l'animal est un accelerometre vert bleu v6","Le marquage de l'animal est un accelerometre vert vert s9","Le marquage de l'animal est un accelerometre vert orange v3 ","L'animal ne porte pas de collier","Le marquage de l'animal est un accelerometre vert orange s7","Le marquage de l'animal est un collier blanc et fagnon blanc","Le marquage de l'animal est un accelerometre vert blanc v5","Le marquage de l'animal est un accelerometre noir vert s2",'Date du jour du releve',NA,NA,'Posture','Comportement','Condition physique',NA,NA,'Blessure observe','Localisation de la blessure','Feces',NA,NA,'Salissure arriere train','Secoue oreille','De quel cote',NA,NA,'Mouche/eternue',NA,NA,'Description complementaire','Remarque generale',NA,NA,'Photo',NA)
v4 <- rep(NA,length(v1))
v5 <- rep(NA,length(v1))
v6 <- c(NA,NA,NA,NA,'yes','yes',NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,'yes',NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA) #,NA,NA,NA,NA);
v7 <- c(NA,NA,NA,NA,NA,NA,'${enclos}="0"','${enclos}="1"','${enclos}="2"','${enclos}="3"','${enclos}="4"','${enclos}="5"','${enclos}="6"','${enclos}="7"','${enclos}="8"','${enclos}="9"',NA,NA,'${nom_0}="212" or ${nom_2}="176" or  ${nom_6}="119" or  ${nom_6}="124" ','${nom_0}= "207"  or ${nom_0}="209" or ${nom_1}="126" or  ${nom_1}="132" or ${nom_3}="182" or ${nom_3}="189" or ${nom_3}="183" or ${nom_4}="171" or ${nom_4}="170" or ${nom_5}="167" or ${nom_5}="173" or ${nom_5}="164" or ${nom_7}="200" or ${nom_7}="197" or ${nom_8}="117" or ${nom_8}="121" or ${nom_9}="56" or ${nom_9}="174"','${nom_1}="126"','${nom_1}="132"','${nom_3}="182"','${nom_3}="189"','${nom_3}="183"','${nom_5}="173"','${nom_5}="164"','${nom_7}="200"','${nom_8}="117"','${nom_8}="121"',NA,NA,NA,NA,'not(${posture}="pas_vu")','not(${posture}="pas_vu")',NA,NA,'not(${posture}="pas_vu")','(not(${blessure}="pas_vu_4" or ${blessure}="aucune")) and (not(${posture}="pas_vu" ))','not(${posture}="pas_vu")',NA,NA,'not(${posture}="pas_vu")','not(${posture}="pas_vu")','${secoue}="yes" and not(${posture}="pas_vu")',NA,NA,'not(${posture}="pas_vu")',NA,NA,NA,NA,NA,NA,NA,NA) #,NA,NA,NA,NA)
v8 <- rep(NA,length(v1))
v9 <- rep(NA,length(v1))
v10 <-rep(NA,length(v1))
v11 <- c(NA,NA,NA,'field-list','minimal','minimal','minimal','minimal','minimal','minimal','minimal','minimal','minimal','minimal','minimal','minimal',NA,'field-list',NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,'field-list','minimal','minimal','minimal',NA,'field-list','minimal','minimal','minimal',NA,'field-list','minimal','minimal','minimal',NA,'field-list','minimal',NA,'field-list',NA,NA,NA,'field-list',NA,NA);
